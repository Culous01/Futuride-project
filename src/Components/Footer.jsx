...
const Footer = () => {
    ...
    const { homeRef, scrollTo } = useScroll();
        ...
        <section className='flex flex-col lg:p-15 p-8 gap-10'>
            <div className='grid lg:grid-cols-3 md:grid-cols-2 gap-5'>
                <div className='flex flex-col lg:gap-16 '>
                    <a onClick={ () => scrollTo(homeRef)}><img src={footerLogo} className='w-20 cursor-pointer' alt="" /></a>
                </div>
                ...
                <div className='lg:col-span-2 flex lg:space-x-60 lg:justify-start justify-between'>
                    <div className='flex flex-col gap-3'>
                        ...
                        {follow.map((item) => 
                            <a key={item.name} className='font-IBM-Plex font-semibold cursor-pointer text-gray-500'>{item.name}</a>
                        )}
                    </div>
